--select
--로그인한 사람의 카트리스트
select * from cart where user_id='guard1';
select * from cart c join product p on c.product_no=p.product_no where c.user_id='guard1';
select * from cart c join userinfo u on u.user_id=c.user_id where c.user_id='guard1';
